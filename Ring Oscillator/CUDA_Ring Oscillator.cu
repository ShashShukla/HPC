...
__device__ void cmos_ring_osc_lhs(int p, int q, int r, int s, float lhs[5][5], float V1, float V2, float V3, float V4, float C1, float C2, float C3, float delta_t, float V_T_p, float V_T_n, float K_p, float K_n, float lambda) {

	//Perturb the parameters
	float V_T_p1 = V_T_p * (0.99 + 0.01 * threadIdx.x);
	float K_p1 = K_p / (0.99 + 0.01 * threadIdx.y);
	float V_T_n1 = V_T_n * (0.99 + 0.01 * blockIdx.x);
	float K_n1 = K_n / (0.99 + 0.01 * blockIdx.y);
	float V_T_p2 = V_T_p * (0.99 + 0.01 * (p / 3));
	float K_p2 = K_p / (0.99 + 0.01 * (p % 3));
	float V_T_n2 = V_T_n * (0.99 + 0.01 * (q / 3));
	float K_n2 = K_n / (0.99 + 0.01 * (q % 3));
	float V_T_p3 = V_T_p * (0.99 + 0.01 * (r / 3));
	float K_p3 = K_p / (0.99 + 0.01 * (r % 3));
	float V_T_n3 = V_T_n * (0.99 + 0.01 * (s / 3));
	float K_n3 = K_n / (0.99 + 0.01 * (s % 3));
	...
	gm_p1 = pMOS_didbydvsg(V1, V4, V2, V_T_p1, K_p1, lambda);
	go_p1 = pMOS_didbydvsd(V1, V4, V2, V_T_p1, K_p1, lambda);
	gm_n1 = nMOS_didbydvgs(0, V4, V2, V_T_n1, K_n1, lambda);
	go_n1 = nMOS_didbydvds(0, V4, V2, V_T_n1, K_n1, lambda);

	gm_p2 = pMOS_didbydvsg(V1, V2, V3, V_T_p2, K_p2, lambda);
	go_p2 = pMOS_didbydvsd(V1, V2, V3, V_T_p2, K_p2, lambda);
	gm_n2 = nMOS_didbydvgs(0, V2, V3, V_T_n2, K_n2, lambda);
	go_n2 = nMOS_didbydvds(0, V2, V3, V_T_n2, K_n2, lambda);

	gm_p3 = pMOS_didbydvsg(V1, V3, V4, V_T_p3, K_p3, lambda);
	go_p3 = pMOS_didbydvsd(V1, V3, V4, V_T_p3, K_p3, lambda);
	gm_n3 = nMOS_didbydvgs(0, V3, V4, V_T_n3, K_n3, lambda);
	go_n3 = nMOS_didbydvds(0, V3, V4, V_T_n3, K_n3, lambda);
...
__global__ void kernel(int p, int q, int r, int s, float* V2d, float* V3d, float* V4d) {

	//Thread ids.
	int x = blockIdx.x * blockDim.x + threadIdx.x;
	int y = blockIdx.y * blockDim.y + threadIdx.y;
	int pos = (blockDim.x * gridDim.x) * y + x; //The 1D index of this thread
	...
	enum { final_T = 15 };      //Total simulation time steps
	...
	for (int i = 0; i < final_T; i++) {
		...
		while ((j < max_iter) && (err > 1e-2)) {
			...
			//LHS and RHS
			cmos_ring_osc_lhs(p, q, r, s, lhs, V1, V2_hat, V3_hat, V4_hat, C1, C2, C3, delta_t, V_T_p, V_T_n, K_p, K_n, lambda);
			cmos_ring_osc_rhs(p, q, r, s, rhs, V, V1, V2_prev, V2_hat, V3_prev, V3_hat, V4_prev, V4_hat, ip1_hat, ip2_hat, ip3_hat, in1_hat, in2_hat, in3_hat, C1, C2, C3, delta_t, V_T_p, V_T_n, K_p, K_n, lambda);
			//Solve MNA
			...
			else if (V_sd_p1 < (V_sg_p1 + V_T_p1)) {
				ip1_hat = (1 + lambda * V_sd_p1) * K_p1 * ((V_sg_p1 + V_T_p1) * V_sd_p1 - V_sd_p1 * V_sd_p1 / 2);
			}
			else {
				ip1_hat = (1 + lambda * V_sd_p1) * K_p1 * (V_sg_p1 + V_T_p1) * (V_sg_p1 + V_T_p1) / 2;
			}
			...
			else if (V_sd_p2 < (V_sg_p2 + V_T_p2)) {
				ip2_hat = (1 + lambda * V_sd_p2) * K_p2 * ((V_sg_p2 + V_T_p2) * V_sd_p2 - V_sd_p2 * V_sd_p2 / 2);
			}
			else {
				ip2_hat = (1 + lambda * V_sd_p2) * K_p2 * (V_sg_p2 + V_T_p2) * (V_sg_p2 + V_T_p2) / 2;
			}
			...
			else if (V_sd_p3 < (V_sg_p3 + V_T_p3)) {
				ip3_hat = (1 + lambda * V_sd_p3) * K_p3 * ((V_sg_p3 + V_T_p3) * V_sd_p3 - V_sd_p3 * V_sd_p3 / 2);
			}
			else {
				ip3_hat = (1 + lambda * V_sd_p3) * K_p3 * (V_sg_p3 + V_T_p3) * (V_sg_p3 + V_T_p3) / 2;
			}
			...
			else if (V_ds_n1 < V_gs_n1 - V_T_n1) {
				in1_hat = (1 + lambda * V_ds_n1) * K_n1 * ((V_gs_n1 - V_T_n1) * V_ds_n1 - V_ds_n1 * V_ds_n1 / 2);
			}
			else {
				in1_hat = (1 + lambda * V_ds_n1) * K_n1 * (V_gs_n1 - V_T_n1) * (V_gs_n1 - V_T_n1) / 2;
			}
			...
			else if (V_ds_n2 < V_gs_n2 - V_T_n2) {
				in2_hat = (1 + lambda * V_ds_n2) * K_n2 * ((V_gs_n2 - V_T_n2) * V_ds_n2 - V_ds_n2 * V_ds_n2 / 2);
			}
			else {
				in2_hat = (1 + lambda * V_ds_n2) * K_n2 * (V_gs_n2 - V_T_n2) * (V_gs_n2 - V_T_n2) / 2;
			}
			...
			else if (V_ds_n3 < V_gs_n3 - V_T_n3) {
				in3_hat = (1 + lambda * V_ds_n3) * K_n3 * ((V_gs_n3 - V_T_n3) * V_ds_n3 - V_ds_n3 * V_ds_n3 / 2);
			}
			else {
				in3_hat = (1 + lambda * V_ds_n3) * K_n3 * (V_gs_n3 - V_T_n3) * (V_gs_n3 - V_T_n3) / 2;
			}
		}
		V2_prev = V2[i];
		V3_prev = V3[i];
	...
	for (int i = 1; i < final_T; i++) {//Copy output from this thread to global memory
		V2d[pos * final_T + i] = V2[i];
		V3d[pos * final_T + i] = V3[i];
		V4d[pos * final_T + i] = V4[i];
	}
}
...
//MNA Variables
float V1 = 5;
float V2h[81 * final_T]; //h = host
float V3h[81 * final_T];
float V4h[81 * final_T];
float *V2d, *V3d, *V4d; //d = device


//Launch the kernel and checks for associated errors
cudaError_t launchKernel()
{
	...
	size_t size = 81 * final_T * sizeof(float);
		...
		for (q = 0; q < 9; q++) {
			for (r = 0; r < 9; r++) {
				for (s = 0; s < 9; s++) {
					...
					// Launch kernel
					kernel << < numBlocks, threadsPerBlock >> >(p, q, r, s, V2d, V3d, V4d);
